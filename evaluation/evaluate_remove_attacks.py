...
parser.add_argument(
    "--remove_prob", type=float, default=0.05, help="Prob to remove words"
)
...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            best_beam_removed = remove_words(best_beam_data)
            best_beam_removed_idx = convert_words_to_idx(best_beam_removed)
            best_beam_emb = model_gen.forward_sent(
                best_beam_removed_idx, msgs, args.gumbel_temp, only_embedding=True
            )
            ...
            if args.use_lm_loss:
                hidden = langModel.init_hidden(batch_size)
                lm_targets = best_beam_removed_idx[1 : len(best_beam_removed_idx)]
                lm_targets = lm_targets.view(
                    lm_targets.size(0) * lm_targets.size(1),
                )
                lm_inputs = best_beam_removed_idx[0 : len(best_beam_removed_idx) - 1]
                lm_out, hidden = langModel(lm_inputs, hidden, decode=True)
                lm_loss = criterion_lm(lm_out, lm_targets)
            ...
            if bert_diff[best_beam_idx] < args.bert_threshold:
                output_adv = ""
                for word in best_beam_removed[0]:
                    output_adv = output_adv + word + " "
                sentences = [output_adv, orig_text]
                sbert_embs = sbert_model.encode(sentences)
                meteor_adv = meteor_score([orig_text], output_adv)
