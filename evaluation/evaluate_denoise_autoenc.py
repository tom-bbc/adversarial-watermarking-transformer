...
# denoising
parser.add_argument(
    "--sub_prob", type=float, default=0.1, help="Probability of substituting words"
)
...
def autoenc_greedy(data, batch_size):
    ...
    sent_out_soft = model_autoenc_attack.forward_sent_decoder(
        sent_emb, data, encoder_out, args.gumbel_temp
    )
    ...
    for j in range(1, data.size(0)):
        sent_out_soft = model_autoenc_attack.forward_sent_decoder(
            sent_emb, sent_out, encoder_out, args.gumbel_temp
        )
        sent_out_new = get_idx_from_logits_softmax(sent_out_soft, j + 1, batch_size)
        sent_out = torch.cat(
            (sent_out[0:j, :].view(j, 1), sent_out_new[j, :].view(1, 1)), axis=0
        )
        sent_out = torch.cat(
            (sent_out, torch.zeros((1, 1), dtype=torch.long).cuda()), axis=0
        )
    return sent_out
...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        data, data_noise, targets = get_batch_noise(
            data_source, i, args, ntokens, evaluation=True
        )
        if args.use_elmo:
        ...
        with torch.no_grad():
            ...
            for k in range(0, data.size(0)):
                output_text_adv = (
                    output_text_adv
                    + corpus.dictionary.idx2word[word_idx_adv[k, 0]]
                    + " "
                )
                orig_text = orig_text + corpus.dictionary.idx2word[data[k, 0]] + " "
                noisy_text = (
                    noisy_text + corpus.dictionary.idx2word[data_noise[k, 0]] + " "
                )
    ...
    return (
        ...
        meteor_tot_noise / batch_count,
        l2_distances / batch_count,
        l2_distances_noise / batch_count,
    )
...
val_lm_loss, val_meteor, val_meteor_noise, val_l2_sbert, val_l2_sbert_noise = evaluate(
...
print("-" * 150)
print(
    "| validation | lm loss {:5.2f} | meteor {:5.4f} | meteor noise {:5.4f} | SentBert dist. {:5.4f} | SentBert dist. noise {:5.4f}".format(
        val_lm_loss, val_meteor, val_meteor_noise, val_l2_sbert, val_l2_sbert_noise
    )
)
print("-" * 150)
f.close()
...
test_lm_loss, test_meteor, test_meteor_noise, test_l2_sbert, test_l2_sbert_noise = (
...
print("=" * 150)
print(
    "| test | lm loss {:5.2f} | meteor {:5.4f} | meteor noise {:5.4f} | SentBert dist. {:5.4f} | SentBert dist. noise {:5.4f}".format(
        test_lm_loss, test_meteor, test_meteor_noise, test_l2_sbert, test_l2_sbert_noise
    )
)
print("=" * 150)
