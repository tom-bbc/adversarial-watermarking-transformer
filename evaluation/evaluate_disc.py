...
parser.add_argument(
    "--disc_path", type=str, default=randomhash + ".pt", help="path to the classifier"
)
...
corpus = data_disc_lm.Corpus("datasets/data_classifier")
...
def evaluate(data_source_real, data_source_fake, batch_size=10):
    ...
    for i in range(0, data_source_real.size(0) - args.bptt, args.bptt):
        data_real, targets_real = get_batch_no_msg(
            data_source_real, i, args, evaluation=True
        )
        data_fake, targets_fake = get_batch_no_msg(
            data_source_fake, i, args, evaluation=True
        )
    ...
    return (
        ...
        fake_correct / batches_count,
        real_correct / batches_count,
        Fscore,
    )
...
val_disc_loss, val_correct_fake, val_correct_real, val_Fscore = evaluate(
    val_real_data, val_fake_data, eval_batch_size
)

print("-" * 150)
print(
    "| validation | disc loss {:5.2f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f}".format(
        val_disc_loss, val_correct_fake * 100, val_correct_real * 100, val_Fscore
    )
)
print("-" * 150)


test_disc_loss, test_correct_fake, test_correct_real, test_Fscore = evaluate(
    test_real_data, test_fake_data, test_batch_size
)

print("-" * 150)
print(
    "| Test | disc loss {:5.2f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f}".format(
        test_disc_loss, test_correct_fake * 100, test_correct_real * 100, test_Fscore
    )
)
print("-" * 150)
