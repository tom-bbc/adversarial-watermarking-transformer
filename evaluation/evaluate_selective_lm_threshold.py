...
parser.add_argument(
    "--lm_threshold", type=float, default=0.45, help="Threshold on the lm difference"
)
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            if lm_loss_diff < args.lm_threshold:
                ...
                out_file.write(
                    str(compare_msg_bits(msgs, torch.round(sig(msg_out_random)))) + "\n"
                )
                out_file.write(
                    np.array2string(msgs.detach().cpu().numpy().astype(int)) + "\n"
                )
                out_file.write(
                    np.array2string(
                        torch.round(sig(msg_out_random))
                        .detach()
                        .cpu()
                        .numpy()
                        .astype(int)
