    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            for beam in range(0, args.samples_num):
                ...
                for k in range(0, data.size(0)):
                    word_idx_dae[k] = corpus_dae.dictionary.word2idx[
                        corpus.dictionary.idx2word[word_idx[k, 0]]
                    ]
            ...
            if len(lm_loss_beams) > 0:
                ...
                for k in range(0, data.size(0)):
                    word_idx_dae[k] = corpus_dae.dictionary.word2idx[
                        corpus.dictionary.idx2word[data[k, 0]]
                    ]
                word_idx_dae_beams.append(word_idx_dae)
                best_beam_idx = 0
            ...
            meteor_score_selected = meteor_score(
                [orig_text], output_text_beams[best_beam_idx]
            )
            ...
            for k in range(0, data.size(0)):
                data_adv_autoenc[k] = corpus.dictionary.word2idx[
                    corpus_dae.dictionary.idx2word[data_adv_autoenc_dae[k, 0]]
                ]

            data_dv_autoenc_emb = model_gen.forward_sent(
                data_adv_autoenc, msgs, args.gumbel_temp, only_embedding=True
            )
