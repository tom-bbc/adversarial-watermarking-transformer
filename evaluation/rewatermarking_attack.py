...
import training.model_mt_autoenc_cce as model_mt_autoenc_cce
...
parser.add_argument(
    ...
    help="path to the second generator",
)
# language loss
parser.add_argument(
...
parser.add_argument(
    "--bert_threshold", type=float, default=30, help="Threshold on the bert distance"
)

parser.add_argument(
    "--samples_num", type=int, default=10, help="samples number during inference"
)
parser.add_argument(
    "--samples_num_adv",
    type=int,
    default=10,
    help="samples number during inference of the adv model",
)
...
def noisy_sampling(sent_encoder_out, both_embeddings, data, model, samples_num):
    candidates_emb = []
    candidates_one_hot = []
    candidates_soft_prob = []
    for i in range(0, samples_num):
        with torch.no_grad():
            sent_out_emb, sent_out_hot, sent_out_soft = model.forward_sent_decoder(
...
def select_best_sample_lm(data, candidates_soft_prob, samples_num, batch_size=1):
    output_text_beams = []
    word_idx_beams = []
    lm_loss_beams = []
    for beam in range(0, samples_num):
    ...
    return best_beam_idx, word_idx_beams, output_text_beams, lm_loss_beams, orig_text
...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        if args.use_lm_loss:
            hidden = langModel.init_hidden(batch_size)
        with torch.no_grad():
            both_embeddings1, sent_encoder_out1 = model_gen.forward_sent_encoder(
                data, msgs, args.gumbel_temp
            )
            candidates_emb, candidates_one_hot, candidates_soft_prob = noisy_sampling(
                sent_encoder_out1, both_embeddings1, data, model_gen, args.samples_num
            )

            # find the best sample of model1
            (
                best_beam_idx1,
                word_idx_beams1,
                output_text_beams1,
                lm_loss_beams1,
                orig_text,
            ) = select_best_sample_lm(data, candidates_soft_prob, args.samples_num)
            best_beam_data1 = word_idx_beams1[best_beam_idx1]

            ###feed the best sample to the other autoencoder
            both_embeddings2, sent_encoder_out2 = model_gen2.forward_sent_encoder(
                best_beam_data1, msgs, args.gumbel_temp
            )
            candidates_emb2, candidates_one_hot2, candidates_soft_prob2 = (
                noisy_sampling(
                    sent_encoder_out2,
                    both_embeddings2,
                    best_beam_data1,
                    model_gen2,
                    args.samples_num_adv,
                )
            )

            # find the best sample of model2
            (
                best_beam_idx2,
                word_idx_beams2,
                output_text_beams2,
                lm_loss_beams2,
                orig_text2,
            ) = select_best_sample_lm(
                best_beam_data1, candidates_soft_prob2, args.samples_num_adv
            )

            ##bert diff using the new sample
            sentences = [output_text_beams2[best_beam_idx2], orig_text]
            sbert_embs = sbert_model.encode(sentences)
            bert_diff_selected = np.linalg.norm(sbert_embs[0] - sbert_embs[1])
            meteor_score_selected = meteor_score(
                [orig_text], output_text_beams2[best_beam_idx2]
            )

            # decode the msg using the firts model
            best_beam_data2 = word_idx_beams2[best_beam_idx2]
            best_beam_emb2 = model_gen.forward_sent(
                best_beam_data2, msgs, args.gumbel_temp, only_embedding=True
            )
            msg_out = model_gen.forward_msg_decode(best_beam_emb2)
            ...
            out_file.write(output_text_beams1[best_beam_idx1] + "\n")
            out_file.write(output_text_beams2[best_beam_idx2] + "\n")
            out_file.write(
            ...
            total_loss_lm += lm_loss_beams2[best_beam_idx2]
            lm_loss_selected = lm_loss_beams2[best_beam_idx2]
        ...
        f_metrics.write(
            str(meteor_score_selected)
            + ","
            + str(bert_diff_selected)
            + ","
            + str(lm_loss_selected)
            + "\n"
        )
...
with open(args.gen_path2, "rb") as f:
    model_gen2, _, _, _ = torch.load(f)
...
print("-" * 150)
print(
    "| validation | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} |  meteor {:5.4f} | SentBert dist. {:5.4f} | P-value {:.9f} | P-value inst {:5.2f}".format(
        val_lm_loss,
        val_correct_msg * 100,
...
print("=" * 150)
print(
    "| test | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | meteor {:5.4f} | SentBert dist. {:5.4f} | P-value {:.9f} | P-value inst {:5.2f}".format(
        test_lm_loss,
