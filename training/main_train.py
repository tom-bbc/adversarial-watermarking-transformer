...
def learing_rate_disc_scheduler():
    ...
    lr = np.power(d_model, -1.1) * min(
        np.power(discr_step_num, -0.5), discr_step_num * np.power(warm_up, -1.5)
    )
    return lr
...
def train():
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        if not args.fixed_length:
            ...
            data, msgs, targets = get_batch_different(
                train_data, i, args, all_msgs, seq_len=seq_len
            )
        else:
            seq_len = args.bptt
            data, msgs, targets = get_batch_different(
                train_data, i, args, all_msgs, seq_len=None
            )
        ...
        if args.use_semantic_loss:
            ...
            errG = (
                args.gen_weight * errG_disc
                + args.msg_weight * errG_msg
                + args.sem_weight * errG_sem
                + args.reconst_weight * errG_reconst
            )
