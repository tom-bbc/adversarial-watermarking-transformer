...
class AutoencModel(nn.Module):
    ...
    def forward_sent_encoder(self, input):
        emb = embedded_dropout(
            self.embeddings, input, dropout=self.dropoute if self.training else 0
        ) * math.sqrt(self.ninp)
    ...
    def forward_sent_decoder(
        self, sent_embedding, input_data, sent_encoder_out, gumbel_temp
    ):
        ...
        sent_embeddings_repeat = sent_embedding.view(
            1, sent_embedding.size(0), sent_embedding.size(1)
    ...
    def forward_sent(self, input_dec, input_noise, gumbel_temp):
        ...
        sent_decoded_vocab_soft = self.forward_sent_decoder(
            sent_embedding, input_dec, encoder_out, gumbel_temp
        )
