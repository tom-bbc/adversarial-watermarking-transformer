...
from data_processing.utils import batchify, generate_msgs, get_batch_noise
...
def learing_rate_scheduler():
    ...
    lr = np.power(d_model, -1.1) * min(
        np.power(step_num, -0.5), step_num * np.power(warm_up, -1.5)
    )
    return lr
...
if args.resume:
    ...
    model_gen = model_denoise_autoenc_attack.AutoencModel(
        ntokens,
        args.emsize,
...
def evaluate(data_source, batch_size=10):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        out_data_prob = model_gen.forward_sent(data, data_noise, args.gumbel_temp)

        # reconstruction loss
        reconst_loss = criterion_reconst(out_data_prob, data.view(-1))
...
def train():
    ...
    while i < train_data.size(0) - 1 - 1:
        if not args.fixed_length:
            ...
            data, data_noise, targets = get_batch_noise(
                train_data, i, args, ntokens, seq_len=seq_len
            )
        else:
            seq_len = args.bptt
            data, data_noise, targets = get_batch_noise(
                train_data, i, args, ntokens, seq_len=None
            )
        model_gen.train()
        ...
        errG_reconst = criterion_reconst(out_data_prob, data.view(-1))
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print(
                "| epoch {:3d} | {:5d}/{:5d} batches | gen lr {:05.8f} | "
...
try:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if "t0" in optimizer_gen.param_groups[0]:
            ...
            val_loss_reconst2 = evaluate(val_data, eval_batch_size)
            val_loss_gen_tot2 = val_loss_reconst2

            print("-" * 89)
            print(
                "| end of epoch {:3d} | time: {:5.2f}s | val reconst loss {:5.2f}".format(
            ...
            val_loss_reconst = evaluate(val_data, eval_batch_size)
...
# Run on test data.
test_loss_reconst = evaluate(test_data, test_batch_size)
