...
parser.add_argument(
    "--encoding_layers", type=int, default=6, help="The number of encoding layers"
)
...
parser.add_argument(
    "--lstm_dim", type=int, default=512, help="The dim of the discriminator lstm"
)
parser.add_argument(
    "--dropouth", type=float, default=0.2, help="dropout on the lstm output"
)
parser.add_argument(
    "--dropout", type=float, default=0.4, help="dropout before the last ff layer"
)
...
if args.resume != "":
    ...
    if args.classifier == "lstm":
        ...
        model_disc = model_discriminator.DiscriminatorModel(
            ntokens,
            args.emsize,
...
def train():
    ...
    while i < train_real_data.size(0) - 1 - 1:
        if not args.fixed_length:
            ...
            data_real, targets_real = get_batch_no_msg(
                train_real_data, i, args, seq_len=seq_len
            )
            data_fake, targets_fake = get_batch_no_msg(
                train_fake_data, fake_batch_itr, args, seq_len=seq_len
            )
        else:
            seq_len = args.bptt
            data_real, targets_real = get_batch_no_msg(
                train_real_data, i, args, seq_len=None
            )
            data_fake, targets_fake = get_batch_no_msg(
                train_fake_data, fake_batch_itr, args, seq_len=None
            )
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print(
                "| epoch {:3d} | {:5d}/{:5d} batches | disc lr {:05.9f} | ms/batch {:5.2f} | "
                "disc loss {:5.2f}".format(
                    epoch,
                    batch,
                    len(train_real_data) // args.bptt,
                    optimizer_disc.param_groups[0]["lr"],
