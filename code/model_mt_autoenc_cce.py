...
class TranslatorGeneratorModel(nn.Module):
    ...
    def __init__(
        ...
        self.sent_encoder_layer = nn.TransformerEncoderLayer(
            d_model=ninp, nhead=attention_heads, dropout=transformer_drop
        )
        self.msg_decoder_layer = nn.TransformerEncoderLayer(
            d_model=ninp, nhead=attention_heads, dropout=transformer_drop
        )
        self.sent_dec_encoder_layer = nn.TransformerDecoderLayer(
            d_model=ninp, nhead=attention_heads, dropout=transformer_drop
        )

        self.sent_encoder = nn.TransformerEncoder(
            self.sent_encoder_layer, nlayers_encoder
        )
        self.msg_decoder = nn.TransformerEncoder(
            self.msg_decoder_layer, nlayers_encoder
        )
        self.sent_decoder = nn.TransformerDecoder(
            self.sent_dec_encoder_layer, nlayers_encoder
        )
    ...
    def forward_msg_decode(self, input):
        ...
        msg_dec_avg = m(
            msg_decoder_out.view(
                msg_decoder_out.size(1),
                msg_decoder_out.size(2),
    ...
    def forward_sent_encoder(self, input, msg_input, gumbel_temp, only_embedding=False):
        ...
        m = nn.AdaptiveAvgPool1d(1)
        sent_embedding = m(
            sent_encoder_out.view(
                sent_encoder_out.size(1),
                sent_encoder_out.size(2),
                sent_encoder_out.size(0),
            )
        )
        sent_embedding = sent_embedding.view(
            sent_embedding.size(0), sent_embedding.size(1)
        )
    ...
    def forward_sent_decoder(
        self, both_embeddings, input_data, sent_encoder_out, gumbel_temp
    ):
        ...
        input_decoder = torch.zeros(
            [input_data.size(0), input_data.size(1), self.ninp]
        ).float()
        ...
        both_embeddings_repeat = both_embeddings.view(
            1, both_embeddings.size(0), both_embeddings.size(1)
        ).repeat(input_data.size(0), 1, 1)
        input_decoder = input_decoder + both_embeddings_repeat
        input_decoder = self.pos_encoder(input_decoder)

        sent_decoded = self.sent_decoder(
            input_decoder, memory=sent_encoder_out, tgt_mask=mask
        )
        ...
        sent_decoded_vocab_hot = F.gumbel_softmax(
            F.log_softmax(sent_decoded_vocab, dim=-1), tau=gumbel_temp, hard=True
        )
        ...
        sent_decoded_vocab_emb = torch.mm(
            sent_decoded_vocab_hot, self.embeddings.weight
        )
        sent_decoded_vocab_emb = sent_decoded_vocab_emb.view(
            input_decoder.size(0), input_decoder.size(1), input_decoder.size(2)
        )
        ...
        return (
            sent_decoded_vocab_emb,
            sent_decoded_vocab_hot_out,
            sent_decoded_vocab_soft,
        )

    def forward_sent(self, input, msg_input, gumbel_temp, only_embedding=False):
        if only_embedding:
            return self.forward_sent_encoder(
                input, msg_input, gumbel_temp, only_embedding=True
            )
        sent_msg_embedding, encoder_out = self.forward_sent_encoder(
            input, msg_input, gumbel_temp, only_embedding=False
        )
        sent_decoded_vocab_emb, sent_decoded_vocab_hot, sent_decoded_vocab_soft = (
            self.forward_sent_decoder(
                sent_msg_embedding, input, encoder_out, gumbel_temp
            )
        )
...
class TranslatorDiscriminatorModel(nn.Module):
    def __init__(
        ...
        self.transformer_encoder = nn.TransformerEncoder(
            self.encoder_layer, nlayers_encoder
        )
    ...
    def forward(self, input_emb):
        ...
        m = nn.AdaptiveAvgPool1d(1)
        disc_avg = m(
            discr_encoder_out.view(
                discr_encoder_out.size(1),
                discr_encoder_out.size(2),
